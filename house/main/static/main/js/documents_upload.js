...
function make_preview_field_row(container, url, name, size, thumbnail) {
  ...
  $('<td></td>')
    ...
    .appendTo(row);
  $('<td></td>')
    .append($('<span>').addClass('filename').text(name))
    .append($('<input type="hidden" name="filenames">').val(name))
    .appendTo(row);
  $('<td></td>')
    .append($('<input name="titles" placeholder="Optional document title">'))
    .appendTo(row);
  $('<td></td>')
...
function guess_file_titles() {
  var f = $('form.upload tbody');
  $('tr', f).each(function(i, row) {
    if (!$('input[name="titles"]', row).val()) {
      var filename = $('span.filename', row).text();
      filename = filename.replace(/_/g, ' ');
      filename = filename.replace(/\.[a-z]{2,4}$/, '');
      filename = filename.replace(/([a-z])([A-Z])/, '$1 $2');
      $('input[name="titles"]', row).val(filename);
    }
  });
}
  ...
  filepicker.pickMultiple(
    ...
    function(InkBlobs){
      ...
      $.each(InkBlobs, function(i, InkBlob) {
        ...
        make_preview_field_row(container, InkBlob.url, InkBlob.filename, InkBlob.size);
      });
      $('form.upload:hidden').show();
      // do this with a slight delay so it's obvious what happens
      setTimeout(function() {
        guess_file_titles();
      }, 2 * 1000);
    },
